...
parser.add_argument('--layerensemble', type=bool,
                    default=False, help='Using layer ensembel')
...
def main():
    ...
    if args.checkpoint is None:
        args.checkpoint = os.path.join("save", args.dataset, args.arch,
                                       f"ep{args.epochs}seed{args.manualSeed}")
    ...
    if args.resume or args.evaluate:
        ...
        logger.set_names(['TAcc_1', 'VAcc_1', 'TAcc_2', 'VAcc_2',
                          'TAcc_3', 'VAcc_3', 'TAcc_e', 'VAcc_e'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc_1, train_acc_2, train_acc_3, train_acc_en = train(trainloader, model, criterion, criterion_kl, optimizer, epoch,
                                                                                use_cuda)
        test_loss, test_acc_1, test_acc_2, test_acc_3, test_acc_en = test(
            testloader, model, use_cuda)

        # append logger file
        logger.append([train_acc_1, test_acc_1, train_acc_2, test_acc_2,
                       train_acc_3, test_acc_3, train_acc_en, test_acc_en])
...
def train(trainloader, model, criterion, criterion_kl, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss_kl = criterion_kl(outputs1, outputs4) + criterion_kl(outputs2,
                                                                  outputs4)+criterion_kl(outputs3, outputs4)
...
def test(testloader, model, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        top1_c3.update(prec1_c3.item(), inputs.size(0))
        top1_avg.update(
            (prec1_c1.item()+prec1_c2.item()+prec1_c3.item())/3, inputs.size(0))
